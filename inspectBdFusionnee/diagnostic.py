...
try:
    missing_authors = pd.read_sql("""
        ...
        WHERE a.id IS NULL;
    """, conn)["nb"][0]
    print(f"Auteurs manquants dans article_authors : {missing_authors}")
    ...
    missing_articles = pd.read_sql("""
        ...
        WHERE ar.id IS NULL;
    """, conn)["nb"][0]
    print(f"Articles manquants dans article_authors : {missing_articles}")
    ...
    print("Erreur dans la vérification des liens :", e)
